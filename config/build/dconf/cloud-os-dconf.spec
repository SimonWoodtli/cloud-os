...
mkdir -p -m0755 %{buildroot}%{_sysconfdir}/dconf/db/local.d %{buildroot}%{_sysconfdir}/dconf/profile %{buildroot}%{_sysconfdir}/systemd/system
install -Dm644 %{SOURCE0} %{SOURCE1} %{SOURCE2} %{buildroot}%{_datadir}/%{VENDOR}/%{NAME}
install -Dm644 %{SOURCE0} %{buildroot}%{_sysconfdir}/dconf/db/local.d
install -Dm644 %{SOURCE1} %{buildroot}%{_sysconfdir}/dconf/profile
install -Dm644 %{SOURCE2} %{buildroot}%{_sysconfdir}/systemd/system

%files
%dir %attr(0755,root,root) %{_datadir}/%{VENDOR}/%{NAME}
%attr(0644,root,root) %{_datadir}/%{VENDOR}/%{NAME}/01-cloud-os
%attr(0644,root,root) %{_datadir}/%{VENDOR}/%{NAME}/user
...
%attr(0644,root,root) %{_sysconfdir}/dconf/profile/user
